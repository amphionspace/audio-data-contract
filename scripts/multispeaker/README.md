...
总数应能被 12 整除；这是独立混音数量，不等于独立原始语音数量。
...
每条混音包含 3～5 个不同 speaker ID；每人有机会连续放入两条不同原句，句间
...
现有只支持 ASR/TS-ASR 的训练入口需要增加该任务渲染后才能训练。
...
错开组要求活动帧中 15%～65% 至少两人活动；密集组要求至少 65%，并实际存在
...
每个成品最多尝试 100 个候选，失败时记录错误并停止继续提交新工作。
...
实现检查：`pytest -q tests/test_multispeaker_synthesis.py`，覆盖 speaker 划分、
严格 clean 标记、越界标签、3/4/5 人完整输出、两句同人归属、重采样，以及根据
源引用和增益重新构造波形并与成品逐样本比较。
