...
def main():
    ...
    for name, kind, path in [
        ...
        item = {'name': name,'kind': kind,'root_alias': 'amphion_data','relative_path': str(path.relative_to(args.amphion_data.resolve())),'expected_bytes': path.stat().st_size,'sha256': hashlib.sha256(path.read_bytes()).hexdigest()}
    ...
    stats={'recordings': len(recs),'supervisions': len(sups),'duration_hours': coverage['duration_hours'],'duration_basis': 'sum_of_recording_durations','source_commands': 205,'minimum_voices_per_command': coverage['minimum_voices'],'speakers': len({r['speaker'] for r in sups})}
    spec={'schema_version': 'dataset-catalog/1.0','dataset_id': 'police_v6_acceptance_205','version': 'v6-acceptance-20260921','languages': ['zh'],'tasks': ['asr'],'artifacts': artifacts,
        'splits': {'test':{'recordings_artifact': 'test_recordings','supervisions_artifact': 'test_supervisions','statistics': stats}},
        'provenance': {'source': 'AmphionData synthetic speech pipeline','status': 'ready','source_requirements': {'filename':'警言警语-V6.0.txt','sha256':plan['source_sha256'],'section':'20260915新增'},'evaluation_limitations': plan['policy'],'human_review': False},
        'recipe_parameters': {'tts_provider': 'qwen3_tts','sample_rate': 16000,'channels': 1}}
    ...
    target=repo/'catalog/police_v6_acceptance_205.yaml'
    if target.exists(): raise FileExistsError(target)
    write_declarations(target, [spec,view])
