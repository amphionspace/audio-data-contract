...
[机器可读登记](../../catalog/local_lhotse_derived.yaml)补充了引擎、train 统计、test 报告及清单落地差异。[逻辑视图](../../views/local_lhotse_views.yaml)仍保留 `inferred` 血缘状态：本次检查没有补齐整个历史流程的工具版本、父版本哈希和全部变换证据。热词派生版未在本次重新扫描，不能把清洗清单的统计直接套用到热词文件。
