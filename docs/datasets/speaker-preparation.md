...
本批已完成处理和登记。各 split 的实测时长、记录数和说话人数见 catalog 和[数据总览](data-overview.md)。源包获取记录见[下载与校验](../history/speaker-data-20260911.md)。
...
以上 View version 均为 `speaker-records-v1-20260912`，血缘为 `exact`。可用入口位于 `views/speaker.yaml`。未处理完成的数据不会提前出现在该文件中。
