...
所有数据注册已迁至 [contract 运行快照](../integrations/icefall-runtime.md)，本仓库维护下载、清洗和准备实现，icefall 消费产物。
...
- `realman` 已在 `audio-data-contract` 的 `icefall_runtime.yaml` 注册，但只能在下面的状态文件显示 `ready` 后纳入训练；仅有压缩包或注册项不代表数据就绪。
