...
Dataset 和 View 声明分别保存在 `catalog/*.yaml`、`views/*.yaml`。每个文件是声明列表，格式示例见 [catalog 说明](../../catalog/README.md)。下文的 JSONL 示例是逐条样本或增量记录。
...
2. 用 `views/local_lhotse_views.yaml` 统一暴露逻辑身份；
